...
class TelegramChannelScraper():
    ...
    async def __verify_channel_name_and_exists(self, channel_name):
        ...
        try:
            ...
            raise Exception(
                "No user or channel with that name! (Try another channel name)")
        return
    ...
    async def __pull_messages_in_batches(self, channel_name, start_message_id, output_folder_name):
        ...
        if start_message_id:
            self.__validate_start_message_id(
                highest_message_id, start_message_id)

        num_batches_need = self.__determine_num_batches_needed(
            highest_message_id, start_message_id)
        ...
        for i in range(1, num_batches_need + 1):
            ...
            async for message in self.client.iter_messages(
                channel_name,
                limit=1000,
                # include message _000 instead of starting at _001
                min_id=batch_low_message_id - 1,
                # include message _999 instead of stopping at __998
                max_id=batch_high_message_id + 1,
                reverse=True,  # oldest to newest
            ):
                raw_batch_messages.append(message)
            parsed_batch_messages = self.__parse_raw_messages(
                raw_batch_messages)
            self.__append_messages_to_file(
                parsed_batch_messages, output_folder_name)
            sleep(30.0)
    ...
    def __validate_start_message_id(self, highest_message_id, start_message_id):
        if start_message_id < 0:
            raise Exception(
                "Provided start message ID is invalid (less than 0)")
        if start_message_id > highest_message_id:
            raise Exception(
                "Provided start message ID is greater than the current highest message ID")
        return
        ...
        try:
            ...
            if message.replies.recent_repliers and len(message.replies.recent_repliers) > 0:
                cleaned['recent_replier_ids'] = [
                    i.user_id for i in message.replies.recent_repliers]
    ...
    def __append_messages_to_file(self, cleaned_messages, output_folder_name):
        ...
        # Otherwise append new data without headers
            ...
            df.to_csv(full_output_file_path, mode="a",
